...
export class ColumnsService {
    ...
    create(
        ...
        return this.db.transaction(async (tx) => {
            ...
            const column = await this.columns.create(
                { boardId, name: dto.name, position },
                tx,
            );

            await this.events.record(
                { boardId, actorId, type: 'column_created', payload: { column } },
                tx,
            );
    ...
    async update(
        columnId: string,
        actorId: string,
        dto: UpdateColumnDto): Promise<Column> {
        return this.db.transaction(async (tx) => {
                ...
                await this.events.record(
                    {
                        ...
                        actorId,
                        type: 'column_renamed',
                        payload: { columnId, name: dto.name },
                    },
                    tx,
                );
            }
    ...
    async remove(columnId: string, actorId: string): Promise<void> {
        await this.db.transaction(async (tx) => {
            ...
            await this.columns.remove(columnId, tx);
            ...
            await this.events.record(
                {
                    ...
                    actorId,
                    type: 'column_deleted',
                    payload: { columnId, name: column.name },
                },
                tx,
            );
        });
